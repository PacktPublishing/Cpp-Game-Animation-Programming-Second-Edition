...
bool VkRenderer::deviceInit() {
  ...
  mMinUniformBufferOffsetAlignment = mPhysDevice.properties.limits.minUniformBufferOffsetAlignment;
...
bool VkRenderer::createChangedPipeline() {
  ...
  if (!Pipeline::init(mRenderData, mRenderData.rdPipelineLayout, mRenderData.rdChangedPipeline, vertexShaderFile, fragmentShaderFile)) {
    Logger::log(1, "%s error: could not init chnaged shader pipeline\n", __FUNCTION__);
    return false;
  }
  return true;
}
...
bool VkRenderer::draw() {
  ...
  mMatrices.projectionMatrix = glm::perspective(90.0f, static_cast<float>(mRenderData.rdVkbSwapchain.extent.width) / static_cast<float>(mRenderData.rdVkbSwapchain.extent.height), 0.1f, 100.f);
