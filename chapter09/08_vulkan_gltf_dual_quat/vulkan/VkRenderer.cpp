...
bool VkRenderer::createGltfPipeline() {
  ...
  if (!GltfPipeline::init(mRenderData, mRenderData.rdPipelineLayout,
...
bool VkRenderer::createGltfSkeletonPipeline() {
  std::string vertexShaderFile = "shader/line.vert.spv";
  std::string fragmentShaderFile = "shader/line.frag.spv";
  if (!GltfSkeletonPipeline::init(mRenderData, mRenderData.rdPipelineLayout,
...
bool VkRenderer::createGltfGPUPipeline() {
  ...
  if (!GltfGPUPipeline::init(mRenderData, mRenderData.rdPipelineLayout,
...
bool VkRenderer::draw() {
  ...
  *mQuatModelMesh = mModel->getVertexData();
  ...
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    ...
    0, nullptr);
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    mRenderData.rdPipelineLayout, 2, 1, &mRenderData.rdJointMatrixSSBO.rdSSBODescriptorSet,
    0, nullptr);
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    mRenderData.rdPipelineLayout, 3, 1, &mRenderData.rdJointDualQuatSSBO.rdSSBODescriptorSet,
    0, nullptr);
