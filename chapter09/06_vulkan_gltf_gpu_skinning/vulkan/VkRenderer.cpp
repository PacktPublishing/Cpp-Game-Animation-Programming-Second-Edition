...
bool VkRenderer::init(unsigned int width, unsigned int height) {
  ...
  if (!createUBO(mRenderData, mRenderData.rdJointMatrixUBO, mGltfModel->getJointMatrices())) {
    return false;
  }
...
bool VkRenderer::createGltfPipeline(VkRenderData& renderData) {
  ...
  if (!GltfPipeline::init(renderData, renderData.rdPipelineLayout,
...
bool VkRenderer::createGltfSkeletonPipeline(VkRenderData& renderData) {
  std::string vertexShaderFile = "shader/line.vert.spv";
  std::string fragmentShaderFile = "shader/line.frag.spv";
  if (!GltfSkeletonPipeline::init(renderData, renderData.rdPipelineLayout,
...
bool VkRenderer::createGltfGPUPipeline(VkRenderData& renderData) {
  ...
  if (!GltfGPUPipeline::init(renderData, renderData.rdPipelineLayout,
...
void VkRenderer::cleanup() {
  ...
  UniformBuffer::cleanup(mRenderData, mRenderData.rdJointMatrixUBO);
...
bool VkRenderer::draw() {
  ...
  *mQuatModelMesh = mModel->getVertexData();
  ...
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    ...
    0, nullptr);
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    mRenderData.rdPipelineLayout, 1, 1, &mRenderData.rdPerspViewMatrixUBO.rdUBODescriptorSet,
    0, nullptr);
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    mRenderData.rdPipelineLayout, 2, 1, &mRenderData.rdJointMatrixUBO.rdUBODescriptorSet,
    0, nullptr);
